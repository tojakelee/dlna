...
static int Net_SockHandshake(int fd, char *host, int port)
{
	...
	char *n_port = "80"; 
	s = getaddrinfo(host, n_port, &hints, &p_res);
	if (s != 0) 
	{
	...
	if( Net_Write( fd, buffer, 9 ) != 9 )
		return -1;
	if( Net_Recv( fd, buffer, 8) != 8 )
		return -1;

	fprintf( stderr, "socks: v=%d cd=%d", buffer[0], buffer[1] );
#endif
	...
	int write_len;
	/*
	write_len = Net_Write( fd, buffer, len ); 
	printf("write len = %d\n", write_len);
	*/
	...
	fprintf( stderr, "socks: v=%x rep=%x atyp=%x\n", buffer[0], buffer[1], buffer[3] );
...
int Net_Connect(char *host, char *port, int timeout)
{
...
next_ai:
	...
	{
		fprintf(stderr, "handshake fail... conection error\n");
		return -1;
	}
...
size_t Net_Recv(int fd, void *buf, size_t buf_len)
{
	size_t recv_size;
...
size_t Net_Write( int fd, void *buf, size_t buf_len)
{
	size_t write_size;
	size_t total;
	int evfd = eventfd (0, EFD_CLOEXEC);
...
//    fd = Net_BindListen("127.0.0.1", "60000",SOCK_STREAM , IPPROTO_TCP);
//	Net_Accept(fd);
//	while(1);

#if 1
	/*
	fd = Net_Connect("127.0.0.1", "60000", 10);
	*/

	fd = Net_Connect("www.google.com", "80", 3000);
	printf("fd = %d\n", fd);
